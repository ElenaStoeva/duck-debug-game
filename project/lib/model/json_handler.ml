...
let rec helper_update list id score level acc =
  ...
  | h::t -> begin 
      if Game.get_id h = id 
      then helper_update t id score level 
          ((Game.create_game id score level (Game.get_levels h))::acc)
...
let game_to_string game = 
  "{ \"id\" : " ^ (Game.get_id game |> Int.to_string )^ ",\n
   \"heighest_score\" : " 
  ^ (Game.get_highest_level game |> Int.to_string )^ ",\n
   \"heighest_level\" : " 
  ^ (Game.get_highest_level game |> Int.to_string )^ ",\n
...
let new_json id score level = 
  ...
  let new_pack =
    ...
    | Some game -> let game_score = Game.get_highest_score game in
      ...
      if game_score < score && game_level < level 
      then update games id score level
      else if Game.get_highest_score game < score 
      then update games id score game_level
      else if Game.get_highest_level game < level 
      then update games id game_score level
      else games
    | None -> games
